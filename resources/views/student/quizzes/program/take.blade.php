...
<style>
    ...
    /* Subtly lift the question the reader is currently on. */
    .question-card.is-active {
        box-shadow: 0 12px 30px -10px rgba(0, 113, 170, 0.25);
    ...
    <div class="grid grid-cols-1 lg:grid-cols-12 gap-6 lg:gap-8">
        ...
        <div class="lg:col-span-8 xl:col-span-9">
            <form id="quizForm" action="{{ route('student.quizzes.program.submit', [$program->id, $quiz->id]) }}" method="POST">
                ...
                <div class="question-card bg-white dark:bg-gray-800 rounded-2xl shadow-lg border border-gray-100 dark:border-gray-700 overflow-hidden mb-6" data-question="{{ $index }}" id="question-{{ $index }}">
                    <!-- Question Header -->
                    ...
                    <div class="px-6 py-5 sm:px-8 sm:py-6">
                        <!-- Question Text -->
                        <div class="mb-6">
                            <p class="text-base sm:text-lg font-medium text-gray-900 dark:text-white leading-relaxed">{{ $question->question_ar }}</p>
                            @if($question->question_en)
                            <p class="mt-1.5 text-sm sm:text-base font-medium text-gray-700 dark:text-gray-300 leading-relaxed" dir="ltr">{{ $question->question_en }}</p>
                            @endif
                            @if($question->image)
                        ...
                        @if($question->type === 'true_false')
                            <div class="flex gap-4 sm:gap-6 flex-wrap">
                                ...
                                <label class="tf-option option-card {{ $optIndex === 0 ? 'true-option' : 'false-option' }} flex items-center justify-center gap-3 p-4 sm:p-5 rounded-2xl cursor-pointer">
                                    <input type="radio"
                                    ...
                                    @endif
                                    <span class="text-base font-bold text-gray-900 dark:text-white">{{ $option->option_ar }}</span>
                                </label>
                                @endforeach
                            </div>
                        @elseif($question->type === 'multiple_choice')
                            <div class="space-y-4">
                                @foreach($question->options as $optIndex => $option)
                                <label class="option-card flex items-center gap-3 p-3.5 sm:p-4 rounded-2xl cursor-pointer">
                                    <input type="radio"
                                           name="answers[{{ $question->id }}][option_id]"
                                           value="{{ $option->id }}"
                                           onchange="selectOption(this, {{ $index }})">
                                    <span class="w-8 h-8 rounded-lg flex items-center justify-center text-xs font-bold bg-gray-100 dark:bg-gray-700 text-gray-600 dark:text-gray-300 flex-shrink-0">
                                        {{ ['أ', 'ب', 'ج', 'د', 'هـ'][$optIndex] ?? ($optIndex + 1) }}
                                    </span>
                                    <span class="text-gray-900 dark:text-white text-base flex-1">{{ $option->option_ar }}</span>
                                </label>
                                @endforeach
                            </div>
                        @elseif($question->type === 'short_answer')
                            <div>
                                <input type="text"
                                       ...
                                       class="answer-input w-full rounded-2xl px-4 py-3 text-base bg-white dark:bg-gray-700 text-gray-900 dark:text-white"
                        ...
                        @elseif($question->type === 'essay')
                            <div>
                                <textarea name="answers[{{ $question->id }}][text]"
                                          rows="6"
                                          class="answer-input w-full rounded-2xl px-4 py-3 text-base bg-white dark:bg-gray-700 text-gray-900 dark:text-white resize-none"
                                ...
                                <p class="mt-2 text-sm text-gray-500 dark:text-gray-400">
                                    ...
                                    يتم تصحيح هذا السؤال يدوياً من قبل ال مدرب 
                                </p>
                            </div>
                        @endif
                    </div>
                ...
                <!-- Submit bar at the end of all questions -->
                <div class="bg-white dark:bg-gray-800 rounded-2xl shadow-lg border border-gray-100 dark:border-gray-700 p-6 sm:p-8 flex flex-col sm:flex-row items-center justify-between gap-4">
                    <div class="text-center sm:text-right">
                        <p class="text-lg font-bold text-gray-900 dark:text-white">هل أنهيت جميع الأسئلة؟</p>
                        <p class="text-sm text-gray-500 dark:text-gray-400">راجع إجاباتك ثم اضغط تسليم الاختبار.</p>
                    </div>
                    <button type="button" onclick="confirmSubmit()"
                            class="nav-button w-full sm:w-auto px-8 py-3.5 rounded-xl font-bold text-white shadow-lg"
...
<script>
    ...
    function setCurrent(index) {
        ...
        document.querySelectorAll('.question-nav-btn').forEach(btn => {
            btn.classList.toggle('current', parseInt(btn.dataset.index) === index);
        });
        document.querySelectorAll('.question-card').forEach(card => {
            card.classList.toggle('is-active', parseInt(card.dataset.question) === index);
        });
    }
    ...
    // Scroll-spy: highlight the question currently in view.
    const questionCards = Array.from(document.querySelectorAll('.question-card'));
    const spy = new IntersectionObserver((entries) => {
        entries.forEach(entry => {
            if (entry.isIntersecting) {
                setCurrent(parseInt(entry.target.dataset.question));
            }
        });
    }, { rootMargin: '-45% 0px -45% 0px', threshold: 0 });
    questionCards.forEach(card => spy.observe(card));
