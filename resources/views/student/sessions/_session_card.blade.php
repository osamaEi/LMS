...
<div class="session-card">
    ...
    @if($isLive || (!$isCompleted && $session->zoom_link))
    <div class="session-card-actions">
        @if($session->zoom_link)
        <a href="{{ $session->zoom_link }}" target="_blank" class="btn btn-join" style="{{ !$isLive ? 'background:linear-gradient(135deg,#2563eb,#1d4ed8);' : '' }}">
