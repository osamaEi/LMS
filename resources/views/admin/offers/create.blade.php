...
<style>
    ...
    .f-section-title {
        font-size:.8rem; font-weight:800; color:#6b7280; text-transform:uppercase;
        letter-spacing:.8px; margin-bottom:1.1rem; display:flex; align-items:center; gap:.5rem;
    }
    ...
    .f-input, .f-select, .f-textarea {
        width:100%; padding:.7rem 1rem; border:2px solid #e5e7eb; border-radius:12px;
        font-size:.88rem; color:#374151; background:#fff; transition:border .2s, box-shadow .2s;
        box-sizing:border-box;
    }
    .dark .f-input, .dark .f-select, .dark .f-textarea { background:#374151; border-color:#4b5563; color:#f9fafb; }
    .f-input:focus, .f-select:focus, .f-textarea:focus {
        outline:none; border-color:#0071AA; box-shadow:0 0 0 3px rgba(0,113,170,.1);
    }
    ...
    .f-radio-option {
        flex:1; display:flex; align-items:center; gap:.6rem; padding:.7rem 1rem;
        border:2px solid #e5e7eb; border-radius:12px; cursor:pointer; transition:all .2s;
    }
    .f-radio-option:has(input:checked) { border-color:#0071AA; background:rgba(0,113,170,.06); }
    .dark .f-radio-option { border-color:#4b5563; }
    .dark .f-radio-option:has(input:checked) { border-color:#0071AA; background:rgba(0,113,170,.15); }
    ...
    .upload-zone {
        border:2px dashed #d1d5db; border-radius:14px; padding:1.5rem; text-align:center;
    ...
    #img-preview { display:none; max-width:100%; height:130px; object-fit:cover; border-radius:10px; margin-top:.75rem; }
</style>
@endpush
    ...
    <div class="form-hero">
        <div style="position:relative;z-index:5;display:flex;align-items:center;gap:1rem;">
            ...
            <div>
                ...
                <h1 style="font-size:1.5rem;font-weight:900;color:#fff;margin:0;">إنشاء عرض جديد</h1>
            </div>
        </div>
    </div>
        ...
        <div class="f-card">
            ...
            <div class="f-grid">
                <div class="f-group">
                    ...
                    <input name="title_ar" class="f-input" type="text" value="{{ old('title_ar') }}" placeholder="مثال: خصم نهاية الفصل" required>
                    @error('title_ar')<span class="error-msg">{{ $message }}</span>@enderror
                </div>
                <div class="f-group">
                    <label class="f-label">عنوان العرض (إنجليزي)</label>
                    <input name="title_en" class="f-input" type="text" value="{{ old('title_en') }}" placeholder="Semester End Discount">
                    @error('title_en')<span class="error-msg">{{ $message }}</span>@enderror
                </div>
            </div>
            <div class="f-grid">
                <div class="f-group">
                    <label class="f-label">وصف العرض (عربي)</label>
                    <textarea name="description_ar" class="f-textarea" placeholder="اكتب وصفاً تفصيلياً للعرض...">{{ old('description_ar') }}</textarea>
                </div>
                <div class="f-group">
                    <label class="f-label">وصف العرض (إنجليزي)</label>
                    <textarea name="description_en" class="f-textarea" placeholder="Write offer description in English...">{{ old('description_en') }}</textarea>
                </div>
            </div>
        </div>
            ...
            <div class="f-grid">
                <div class="f-group">
                    ...
                    <select name="program_id" class="f-select">
                        ...
                        <option value="{{ $prog->id }}" {{ old('program_id')==$prog->id?'selected':'' }}>{{ $prog->name_ar }}</option>
                        @endforeach
                    </select>
                    <span class="f-hint">اتركه فارغاً لتطبيق العرض على جميع البرامج</span>
                </div>
                <div class="f-group">
                    <label class="f-label">الحد الأقصى للاستخدامات</label>
                    <input name="max_uses" class="f-input" type="number" min="1" value="{{ old('max_uses') }}" placeholder="مثال: 100">
                    <span class="f-hint">اتركه فارغاً لاستخدام غير محدود</span>
                </div>
            </div>
            <div class="f-grid">
                <div class="f-group">
                    <label class="f-label"><span class="f-required">*</span>تاريخ البداية</label>
                    <input name="start_date" class="f-input" type="date" value="{{ old('start_date', date('Y-m-d')) }}" required>
                ...
                <div class="f-group">
                    ...
                    <input name="end_date" class="f-input" type="date" value="{{ old('end_date') }}" required>
                ...
                <div class="f-group">
                    ...
                    <div class="upload-zone" onclick="this.querySelector('input').click()">
                        ...
                        <div id="upload-placeholder">
                            <svg style="width:36px;height:36px;color:#d1d5db;margin:0 auto .5rem;" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="1.5" d="M7 16a4 4 0 01-.88-7.903A5 5 0 1115.9 6L16 6a5 5 0 011 9.9M15 13l-3-3m0 0l-3 3m3-3v8"/></svg>
                            <p style="font-size:.8rem;color:#9ca3af;margin:0;">اسحب الصورة هنا أو انقر للاختيار</p>
                            <p style="font-size:.7rem;color:#d1d5db;margin:.2rem 0 0;">JPG, PNG, WebP — حد أقصى 2MB</p>
                        </div>
                        <img id="img-preview" alt="معاينة">
                    </div>
                </div>
                <div class="f-group">
