...
<style>
    ...
    .stat-card:hover { transform: translateY(-6px); box-shadow: 0 25px 50px -12px rgba(245,158,11,.25); }
    ...
    .table-row:hover { background: linear-gradient(90deg, rgba(245,158,11,.03) 0%, rgba(245,158,11,.08) 50%, rgba(245,158,11,.03) 100%); }
    .table-row:hover td:first-child { border-right-color: #f59e0b; }

    .action-btn { transition: all .2s ease; position: relative; overflow: hidden; }
    .action-btn::before { content: ''; position: absolute; inset: 0; background: currentColor; opacity: 0; transition: opacity .2s ease; }
    ...
    @keyframes pulse {
        ...
        50%       { opacity: .5; transform: translateY(-50%) scale(1.2); }
    }

    .search-input:focus { box-shadow: 0 0 0 3px rgba(245,158,11,.1); }
    ...
    .custom-checkbox:checked { background: linear-gradient(135deg,#f59e0b,#d97706); border-color: #f59e0b; }
    ...
    .custom-checkbox:hover { border-color: #f59e0b; }
...
<div class="courses-page space-y-6">
            ...
            <div class="flex flex-col lg:flex-row lg:items-center lg:justify-between gap-6">
                ...
                <div class="flex items-center gap-3">
                    <a href="{{ route('admin.courses.create') }}"
                       class="btn-shine inline-flex items-center gap-3 px-6 py-3.5 rounded-2xl bg-white text-[#d97706] font-bold shadow-lg hover:shadow-xl transition-all transform hover:-translate-y-0.5">
    ...
    <div class="grid grid-cols-1 md:grid-cols-3 gap-5">
        <div class="stat-card bg-white dark:bg-gray-800 rounded-2xl shadow-sm p-6" style="--card-accent:#f59e0b;">
            <div class="flex items-start justify-between">
                <div>
                    <p class="text-sm font-medium text-gray-500 dark:text-gray-400">إجمالي الدورات</p>
                    <p class="text-4xl font-bold text-gray-900 dark:text-white mt-2">{{ $stats['total'] }}</p>
                    <p class="text-xs text-gray-400 dark:text-gray-500 mt-2">جميع الدورات في النظام</p>
                </div>
        ...
        <div class="stat-card bg-white dark:bg-gray-800 rounded-2xl shadow-sm p-6" style="--card-accent:#10b981;">
            <div class="flex items-start justify-between">
                <div>
                    <p class="text-sm font-medium text-gray-500 dark:text-gray-400">دورات نشطة</p>
        ...
        <div class="stat-card bg-white dark:bg-gray-800 rounded-2xl shadow-sm p-6" style="--card-accent:#6b7280;">
            <div class="flex items-start justify-between">
                <div>
                    <p class="text-sm font-medium text-gray-500 dark:text-gray-400">دورات غير نشطة</p>
    ...
    <div class="bg-white dark:bg-gray-800 rounded-2xl shadow-sm overflow-hidden">
        <div class="p-6 border-b border-gray-100 dark:border-gray-700">
            <div class="flex flex-col lg:flex-row lg:items-center lg:justify-between gap-4">
                <div class="flex items-center gap-4">
                    <div class="w-12 h-12 rounded-2xl flex items-center justify-center" style="background:linear-gradient(135deg,#f59e0b,#d97706);">
                        <svg class="w-6 h-6 text-white" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M4 6h16M4 10h16M4 14h16M4 18h16"/>
                        </svg>
                    </div>
                    <div>
                        <h2 class="text-xl font-bold text-gray-900 dark:text-white">قائمة الدورات</h2>
                        <p class="text-sm text-gray-500 dark:text-gray-400">{{ $programs->total() }} دورة تدريبية</p>
                    </div>
                </div>
            ...
            <table class="data-table w-full">
                <thead>
                    <tr class="bg-gray-50 dark:bg-gray-700/50">
                        ...
                        <th class="px-6 py-4 text-right text-xs font-bold text-gray-500 dark:text-gray-400 uppercase tracking-wider">الدورة</th>
                    ...
                    <tr class="table-row" data-status="{{ $program->status }}" data-type="{{ $program->course_type }}" data-name="{{ $program->name_ar }} {{ $program->name_en }}">
                        ...
                        <td class="px-6 py-5">
                            <div class="flex items-center gap-4">
                                ...
                                @else
                                <div class="w-12 h-12 rounded-xl flex items-center justify-center flex-shrink-0" style="background:linear-gradient(135deg,#f59e0b20,#d9770620);">
                                    <svg class="w-6 h-6" style="color:#f59e0b;" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 6.253v13m0-13C10.832 5.477 9.246 5 7.5 5S4.168 5.477 3 6.253v13C4.168 18.477 5.754 18 7.5 18s3.332.477 4.5 1.253m0-13C13.168 5.477 14.754 5 16.5 5c1.747 0 3.332.477 4.5 1.253v13C19.832 18.477 18.247 18 16.5 18c-1.746 0-3.332.477-4.5 1.253"/>
                                    </svg>
                                </div>
                                @endif
                        ...
                        <td class="px-6 py-5">
                            <div class="flex items-center justify-center gap-1">
                                ...
                                <form action="{{ route('admin.courses.destroy', $program) }}" method="POST" class="inline"
        ...
        <div class="p-16 text-center">
            <div class="empty-icon relative w-32 h-32 mx-auto mb-8">
                <div class="absolute inset-0 rounded-full bg-gradient-to-br from-amber-100 to-amber-200 dark:from-gray-700 dark:to-gray-800"></div>
                <div class="absolute inset-4 rounded-full bg-white dark:bg-gray-800 flex items-center justify-center shadow-inner">
                    <svg class="w-12 h-12 text-amber-300 dark:text-gray-600" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="1.5" d="M12 6.253v13m0-13C10.832 5.477 9.246 5 7.5 5S4.168 5.477 3 6.253v13C4.168 18.477 5.754 18 7.5 18s3.332.477 4.5 1.253m0-13C13.168 5.477 14.754 5 16.5 5c1.747 0 3.332.477 4.5 1.253v13C19.832 18.477 18.247 18 16.5 18c-1.746 0-3.332.477-4.5 1.253"/>
                    </svg>
                </div>
            </div>
            <h3 class="text-2xl font-bold text-gray-900 dark:text-white mb-3">لا توجد دورات بعد</h3>
            <p class="text-gray-500 dark:text-gray-400 max-w-md mx-auto mb-6">ابدأ بإضافة دورة تدريبية جديدة.</p>
            <a href="{{ route('admin.courses.create') }}"
