...
<div class="mb-6">
    ...
    <p class="text-sm text-gray-500 dark:text-gray-400">أدخل بيانات الدورة والأرباع الدراسية وقم بتعيين المقررات  في خطوة واحدة</p>
</div>
...
<form action="{{ route('admin.courses.store') }}" method="POST" id="courseForm" enctype="multipart/form-data">
            ...
            <div class="grid grid-cols-1 gap-5 md:grid-cols-2">
                ...
                <div class="md:col-span-2">
                    ...
                    <textarea name="description_ar" rows="3"
                              class="w-full rounded-lg border border-gray-300 bg-white px-4 py-2.5 text-sm text-gray-900 focus:border-amber-500 focus:ring-2 focus:ring-amber-500 dark:border-gray-700 dark:bg-gray-800 dark:text-white"
                              placeholder="وصف تفصيلي عن الدورة وأهدافها...">{{ old('description_ar') }}</textarea>
                </div>
                <div class="md:col-span-2">
                    ...
                    <textarea name="description_en" rows="3" dir="ltr"
                              class="w-full rounded-lg border border-gray-300 bg-white px-4 py-2.5 text-sm text-gray-900 focus:border-amber-500 focus:ring-2 focus:ring-amber-500 dark:border-gray-700 dark:bg-gray-800 dark:text-white"
                              placeholder="Detailed description of the course...">{{ old('description_en') }}</textarea>
                </div>
                ...
                <div class="md:col-span-2">
                    ...
                    <label for="programImage" class="block cursor-pointer">
                        <div id="imagePreviewWrap"
                             class="relative w-full rounded-xl border-2 border-dashed border-gray-300 dark:border-gray-600 bg-gray-50 dark:bg-gray-800 overflow-hidden transition-colors hover:border-amber-400"
                             style="height:220px;">
    ...
    <div class="rounded-xl border border-gray-200 bg-white dark:border-gray-800 dark:bg-gray-900 mb-6">
        <div class="p-5 border-b border-gray-200 dark:border-gray-800" style="background:linear-gradient(135deg,#7c3aed,#5b21b6);border-radius:12px 12px 0 0;">
            <div class="flex items-center justify-between">
                <div class="flex items-center gap-3">
                    ...
                    <h2 class="text-base font-bold text-white">الأرباع والمقررات </h2>
                </div>
        ...
        <div id="termsContainer" class="divide-y divide-gray-100 dark:divide-gray-800"></div>
...
function buildTermHTML(idx, num) {
    ...
    return `
        <div class="flex items-center gap-3 mb-4">
            ...
            <h3 class="text-sm font-semibold text-gray-900 dark:text-white">الربع ${num}</h3>
            <button type="button" onclick="removeTerm(${num})" class="mr-auto text-xs text-red-500 hover:text-red-700 flex items-center gap-1">
                <svg class="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor">
        ...
        <div class="grid grid-cols-1 gap-4 sm:grid-cols-2 md:grid-cols-4 mb-4">
            <div>
                ...
                <input type="text" name="terms[${idx}][name_ar]" class="w-full rounded-lg border border-gray-300 bg-white px-3 py-2 text-sm dark:bg-gray-800 dark:border-gray-700 dark:text-white" placeholder="الربع الأول">
            </div>
            <div>
                <label class="block text-xs font-medium text-gray-600 dark:text-gray-400 mb-1">اسم الربع (إنجليزي)</label>
                <input type="text" name="terms[${idx}][name_en]" dir="ltr" class="w-full rounded-lg border border-gray-300 bg-white px-3 py-2 text-sm dark:bg-gray-800 dark:border-gray-700 dark:text-white" placeholder="First Term">
            </div>
            <div>
                <label class="block text-xs font-medium text-gray-600 dark:text-gray-400 mb-1">تاريخ البداية</label>
                <input type="date" name="terms[${idx}][start_date]" class="w-full rounded-lg border border-gray-300 bg-white px-3 py-2 text-sm dark:bg-gray-800 dark:border-gray-700 dark:text-white">
            </div>
            <div>
                <label class="block text-xs font-medium text-gray-600 dark:text-gray-400 mb-1">تاريخ النهاية</label>
                <input type="date" name="terms[${idx}][end_date]" class="w-full rounded-lg border border-gray-300 bg-white px-3 py-2 text-sm dark:bg-gray-800 dark:border-gray-700 dark:text-white">
            </div>
        </div>
        <div class="mb-4">
            <label class="block text-xs font-medium text-gray-600 dark:text-gray-400 mb-1">الحالة</label>
            <select name="terms[${idx}][status]" class="rounded-lg border border-gray-300 bg-white px-3 py-2 text-sm dark:bg-gray-800 dark:border-gray-700 dark:text-white">
        ...
        <div>
            <div class="flex items-center justify-between mb-2">
                <label class="block text-xs font-medium text-gray-600 dark:text-gray-400">المقررات  <span id="selectedCount_${idx}" class="text-purple-600 font-bold">(0 محدد)</span></label>
                <input type="text" oninput="filterSubjects(this, ${idx})" placeholder="بحث..." class="rounded-lg border border-gray-300 bg-white px-3 py-1.5 text-xs dark:bg-gray-800 dark:border-gray-700 dark:text-white w-40">
            </div>
            <div id="chipsContainer_${idx}" class="rounded-lg border border-gray-200 dark:border-gray-700 p-3" style="max-height:180px;overflow-y:auto;">${subjectChips}</div>
