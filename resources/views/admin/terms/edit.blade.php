...
@section('title', 'تعديل الربع الدراسي')
...
<div class="mb-6">
    <div class="flex items-center gap-3 mb-3">
        ...
        <h1 class="text-2xl font-bold text-gray-900 dark:text-white">تعديل الربع الدراسي</h1>
    </div>
    <p class="text-sm text-gray-500 dark:text-gray-400">تعديل بيانات {{ $term->name }}</p>
</div>
        ...
        <div class="grid grid-cols-1 gap-6 md:grid-cols-2">
            ...
            <div class="md:col-span-2">
                <label class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-2">
                    الدبلوم <span class="text-red-500">*</span>
                </label>
                <select name="program_id" required
                    ...
                    @foreach($programs as $program)
                        <option value="{{ $program->id }}" {{ old('program_id', $term->program_id) == $program->id ? 'selected' : '' }}>
            ...
            <div>
                ...
                <input type="number" name="term_number"
            ...
            <div>
                ...
                <select name="status" required
                        class="w-full rounded-lg border border-gray-300 bg-white px-4 py-2.5 text-gray-900 focus:border-brand-500 focus:ring-2 focus:ring-brand-500 dark:border-gray-700 dark:bg-gray-800 dark:text-white">
                    <option value="upcoming"  {{ old('status', $term->status) === 'upcoming'  ? 'selected' : '' }}>قادم</option>
                    <option value="active"    {{ old('status', $term->status) === 'active'    ? 'selected' : '' }}>نشط</option>
                    <option value="completed" {{ old('status', $term->status) === 'completed' ? 'selected' : '' }}>مكتمل</option>
                </select>
            </div>
            ...
            <div>
                ...
                <input type="text" name="name_ar"
                       value="{{ old('name_ar', $term->name_ar) }}"
                       class="w-full rounded-lg border border-gray-300 bg-white px-4 py-2.5 text-gray-900 focus:border-brand-500 focus:ring-2 focus:ring-brand-500 dark:border-gray-700 dark:bg-gray-800 dark:text-white"
            ...
            <div>
                ...
                <input type="text" name="name_en" dir="ltr"
                       value="{{ old('name_en', $term->name_en) }}"
                       class="w-full rounded-lg border border-gray-300 bg-white px-4 py-2.5 text-gray-900 focus:border-brand-500 focus:ring-2 focus:ring-brand-500 dark:border-gray-700 dark:bg-gray-800 dark:text-white"
                       placeholder="Example: First Quarter">
            </div>
        ...
        <div class="mt-6 border-t border-gray-200 dark:border-gray-800 pt-6">
            <div class="flex items-center justify-between mb-3">
                <label class="block text-sm font-medium text-gray-700 dark:text-gray-300">
                    ...
                    <span id="selected-count" class="mr-2 text-xs font-normal text-brand-600">({{ count($assignedSubjectIds) }} محدد)</span>
                </label>
            ...
            <div id="subjects-list"
                ...
                @empty
                <p class="text-sm text-gray-400 p-4 text-center">لا توجد مقررات.</p>
                @endforelse
