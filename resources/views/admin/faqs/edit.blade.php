...
.cat-card .cat-check svg { width:10px; height:10px; }
...
.sort-btn { width:38px; height:38px; border:none; background:transparent; cursor:pointer; color:#64748b; font-size:1.1rem; display:flex; align-items:center; justify-content:center; transition:background .12s; flex-shrink:0; }
...
.preview-q { padding:14px 16px; background:#f0f9ff; border-bottom:1.5px solid #e0f2fe; display:flex; align-items:flex-start; gap:10px; }
...
.btn-save { display:inline-flex; align-items:center; gap:8px; padding:11px 28px; background:linear-gradient(135deg,#0071AA,#005a88); color:#fff; border:none; border-radius:10px; font-size:.875rem; font-weight:800; cursor:pointer; transition:opacity .15s,transform .1s; }
    ...
    <div class="hero-banner">
        <div style="position:relative;z-index:1;display:flex;align-items:center;gap:14px;">
            ...
            <div>
                ...
                <h1 style="font-size:1.25rem;font-weight:800;margin:0 0 3px;">تعديل سؤال شائع</h1>
                <p style="font-size:.8rem;opacity:.75;margin:0;">عدّل البيانات ثم اضغط حفظ — التغييرات تظهر فوراً</p>
            </div>
            <div style="margin-right:auto;background:rgba(255,255,255,.12);border-radius:10px;padding:8px 14px;text-align:center;">
                <div style="font-size:.68rem;opacity:.7;margin-bottom:2px;">رقم السؤال</div>
                <div style="font-size:1.1rem;font-weight:800;">#{{ $faq->id }}</div>
            </div>
        </div>
    </div>
                ...
                <div class="panel">
                    <div class="panel-head" style="background:linear-gradient(135deg,#0071AA,#005a88);border-radius:14px 14px 0 0;">
                        ...
                        <div>
                            ...
                            <div class="panel-head-sub" style="color:rgba(255,255,255,.65);">عدّل المحتوى بالعربية أو الإنجليزية</div>
                        </div>
                        <div style="margin-right:auto;">
                            <div class="lang-tabs">
                                <button type="button" class="lang-tab active" id="tab-ar" onclick="switchLang('ar')">عربي</button>
                                <button type="button" class="lang-tab" id="tab-en" onclick="switchLang('en')">English</button>
                            </div>
                        </div>
                    </div>
                    ...
                    <div class="panel-body">
                        ...
                        <div class="lang-panel active" id="panel-ar">
                            <div style="margin-bottom:16px;">
                                <label class="fl"><span class="req">*</span>السؤال بالعربية</label>
                            ...
                            <div>
                                <label class="fl"><span class="req">*</span>الإجابة بالعربية</label>
                                <textarea name="answer_ar" id="answer_ar" rows="6"
                                          class="fi {{ $errors->has('answer_ar') ? 'is-error' : '' }}"
                                          placeholder="اكتب الإجابة بوضوح..."
                                          oninput="syncPreview()">{{ old('answer_ar', $faq->answer_ar) }}</textarea>
                                @error('answer_ar')
                                <span class="err-msg">
                                    <svg style="width:12px;height:12px" fill="currentColor" viewBox="0 0 20 20"><path fill-rule="evenodd" d="M18 10a8 8 0 11-16 0 8 8 0 0116 0zm-7 4a1 1 0 11-2 0 1 1 0 012 0zm-1-9a1 1 0 00-1 1v4a1 1 0 102 0V6a1 1 0 00-1-1z" clip-rule="evenodd"/></svg>
                                    {{ $message }}
                        ...
                        <div class="lang-panel" id="panel-en">
                            ...
                            <div>
                                ...
                                <textarea name="answer_en" rows="6" class="fi" dir="ltr"
                                          placeholder="Write a clear and complete answer in English...">{{ old('answer_en', $faq->answer_en) }}</textarea>
                    ...
                    <div class="panel-body">
                        <div class="preview-box">
                            <div class="preview-q">
                                ...
                                <div class="pq-text" id="previewQ"></div>
                            </div>
                            <div class="preview-a" id="previewA"></div>
                        </div>
                    </div>
                </div>
                ...
                <div class="panel">
                    ...
                    <div class="panel-body">
                        <div class="cat-grid">
                            ...
                            @php $isSelected = old('category', $faq->category) === $key; @endphp
                    ...
                    <div class="panel-body">
                        <div class="sort-wrap">
                            ...
                            <input type="number" name="sort_order" id="sortInput"
                                   value="{{ old('sort_order', $faq->sort_order) }}" min="0" max="999">
                            <button type="button" class="sort-btn" onclick="changeSort(1)">+</button>
                        </div>
                        ...
                        <div class="status-wrap">
                            ...
                            <div class="status-opt inactive">
                                ...
                                <label for="status_inactive">
                                    <span class="status-dot"></span>مخفي
                                </label>
                            </div>
                        </div>
...
function syncPreview() {
    ...
    document.getElementById('previewQ').innerHTML = q || '<span class="preview-empty">سيظهر نص السؤال هنا...</span>';
    document.getElementById('previewA').innerHTML = a ? a.replace(/\n/g,'<br>') : '<span class="preview-empty">سيظهر نص الإجابة هنا...</span>';
}
...
function changeSort(delta) {
    ...
    inp.value = Math.max(0, Math.min(999, parseInt(inp.value || 0) + delta));
}

function updateStatusHint() {
    const hint = document.getElementById('statusHint');
    const active = document.getElementById('status_active').checked;
    hint.style.background = active ? '#f0fdf4' : '#fef2f2';
    hint.style.color = active ? '#065f46' : '#991b1b';
    hint.textContent = active
        ? '✅ السؤال سيظهر للزوار على صفحة الأسئلة الشائعة.'
        : '🔴 السؤال مخفي ولن يظهر للزوار حتى تغيّر الحالة.';
}
document.querySelectorAll('input[name=status]').forEach(r => r.addEventListener('change', updateStatusHint));
updateStatusHint();

// Init
const qAr = document.getElementById('question_ar');
if (qAr.value) updateCounter(qAr,'cnt-qar',500);
const qEn = document.getElementById('question_en');
if (qEn && qEn.value) updateCounter(qEn,'cnt-qen',500);
syncPreview();
</script>
@endsection
