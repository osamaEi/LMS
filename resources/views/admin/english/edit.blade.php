...
@section('title', 'تعديل دورة اللغة الإنجليزية')
...
<div class="mb-6">
    <div class="flex items-center gap-3 mb-3">
        ...
        <h1 class="text-2xl font-bold text-gray-900 dark:text-white">تعديل دورة اللغة الإنجليزية</h1>
    </div>
    <p class="text-sm text-gray-500 dark:text-gray-400">{{ $program->name_ar }}</p>
</div>
...
<form action="{{ route('admin.english.update', $program) }}" method="POST" enctype="multipart/form-data">
    @csrf
    @method('PUT')
            ...
            <div class="grid grid-cols-1 gap-5 md:grid-cols-2">
                ...
                <div class="md:col-span-2">
                    ...
                    <select name="level" required
                        ...
                        @foreach($levelNames as $num => $label)
                            <option value="{{ $num }}" {{ (string)old('level', $program->level) === (string)$num ? 'selected' : '' }}>
                                {{ $num }} — {{ $label }}
                            </option>
                        @endforeach
                    </select>
                ...
                <div class="md:col-span-2">
                    ...
                    <textarea name="description_ar" rows="3"
                              class="w-full rounded-lg border border-gray-300 bg-white px-4 py-2.5 text-sm text-gray-900 focus:border-blue-500 focus:ring-2 focus:ring-blue-500 dark:border-gray-700 dark:bg-gray-800 dark:text-white">{{ old('description_ar', $program->description_ar) }}</textarea>
                </div>
                <div class="md:col-span-2">
                    ...
                    <textarea name="description_en" rows="3" dir="ltr"
                              class="w-full rounded-lg border border-gray-300 bg-white px-4 py-2.5 text-sm text-gray-900 focus:border-blue-500 focus:ring-2 focus:ring-blue-500 dark:border-gray-700 dark:bg-gray-800 dark:text-white">{{ old('description_en', $program->description_en) }}</textarea>
                </div>
