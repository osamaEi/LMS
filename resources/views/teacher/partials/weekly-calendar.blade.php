...
function openSession(s){
    ...
    if(s.status!=='completed'){
        const hasLink = s.zoom_start_url || s.zoom_join_url;
        // ▶ ابدأ الجلسة — يمر على الـ backend ليسجّل بدء الجلسة (started_at) ثم يفتح رابط البدء.
        // بدء الجلسة شرط لانضمام الطلاب.
        html+=`<a href="${hasLink?('/teacher/sessions/'+s.id+'/start'):'#'}" target="_blank" ${hasLink?'':'onclick="alert(\'لا يوجد رابط بعد\');return false;"'} style="display:flex;align-items:center;justify-content:center;gap:8px;padding:10px;background:linear-gradient(135deg,#ef4444,#dc2626);color:white;border-radius:10px;text-decoration:none;font-size:13px;font-weight:700;${hasLink?'':'opacity:.55;'}">
