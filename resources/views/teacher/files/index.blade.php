...
<div class="container mx-auto px-4 py-6 max-w-7xl" x-data="filesApp()" x-init="init()">
    ...
    <div class="files-hero mb-6">
        <div class="relative z-10" style="display:flex;align-items:center;gap:20px">
            <div style="width:56px;height:56px;border-radius:16px;background:linear-gradient(135deg,#6366f1,#4f46e5);display:flex;align-items:center;justify-content:center;flex-shrink:0;box-shadow:0 8px 24px rgba(99,102,241,0.4)">
                <svg width="28" height="28" viewBox="0 0 24 24" fill="white">
                    <path d="M20 6h-2.18c.07-.44.18-.88.18-1.36C18 2.5 15.5 0 12.36 0c-1.73 0-3.27.82-4.27 2.09L7 3 5.91 2.09C4.91.82 3.37 0 1.64 0-1.5 0-4 2.5-4 5.64c0 .48.11.92.18 1.36H-6v13h26V6zm-9 11H5V8h6v9zm8 0h-6V8h6v9z"/>
                </svg>
            </div>
            <div>
            ...
            <div style="margin-right:auto;display:flex;gap:10px">
                ...
                @endphp
                @foreach([[$totalFiles,'إجمالي الملفات','#6366f1'],[$totalPdfs,'PDF','#dc2626'],[$totalVids,'فيديو','#2563eb']] as [$count,$label,$color])
    ...
    <div style="display:grid;grid-template-columns:260px 1fr;gap:20px;align-items:start">
        ...
        <div style="background:#fff;border-radius:18px;border:1.5px solid #f1f5f9;padding:12px;position:sticky;top:80px" class="dark:bg-slate-800 dark:border-white/10">
            ...
            @endphp
            <a href="{{ route('teacher.files.index', ['subject' => $subject->id]) }}"
               class="subject-tab {{ $isActive ? 'active' : '' }}">
                ...
                <span class="tab-count">{{ $subjectFileCount }}</span>
            </a>
            @empty
        ...
        <div>
            ...
            <div style="margin-bottom:28px">
                ...
                <div style="display:flex;align-items:center;gap:12px;margin-bottom:14px">
                    <div style="width:36px;height:36px;border-radius:10px;background:{{ $subject->color ?? '#6366f1' }};display:flex;align-items:center;justify-content:center;flex-shrink:0">
                ...
                <div style="margin-bottom:16px">
                    <div style="font-size:0.72rem;font-weight:700;color:#6366f1;letter-spacing:0.06em;text-transform:uppercase;margin-bottom:10px;display:flex;align-items:center;gap:6px">
                    ...
                    <div style="display:grid;grid-template-columns:repeat(auto-fill,minmax(280px,1fr));gap:14px">
                        ...
                        <div class="file-card"
                             ...
                             @click="openPreview({
                                 ...
                                 title: @js($file->title),
                                 type: '{{ $isPdf ? 'pdf' : 'doc' }}',
                                 url: @js(asset('storage/' . $file->file_path)),
                                 session: 'ملفات المادة',
                ...
                <div>
                    ...
                    <div style="display:grid;grid-template-columns:repeat(auto-fill,minmax(280px,1fr));gap:14px">
                        ...
                        <div class="file-card"
                             ...
                             @endif>
                            <div style="height:4px;background:{{ $isPdf ? 'linear-gradient(90deg,#dc2626,#ef4444)' : 'linear-gradient(90deg,#2563eb,#3b82f6)' }}"></div>
                            <div style="padding:16px">
                                <div style="display:flex;align-items:flex-start;justify-content:space-between;margin-bottom:12px">
                                    <div style="width:48px;height:48px;border-radius:12px;background:{{ $isPdf ? '#fee2e2' : '#dbeafe' }};display:flex;align-items:center;justify-content:center">
                                        @if($isPdf)
                                        <svg width="24" height="24" viewBox="0 0 24 24" fill="#dc2626">
                                            <path d="M14 2H6a2 2 0 00-2 2v16a2 2 0 002 2h12a2 2 0 002-2V8l-6-6zm-1 1.5L18.5 9H13V3.5zM8 17h8v-1.5H8V17zm0-3h8v-1.5H8V14zm0-3h5v-1.5H8V11z"/>
                                        </svg>
                                    ...
                                    <div style="display:flex;flex-direction:column;align-items:flex-end;gap:4px">
                                        ...
                                        @else
                                        <span style="font-size:0.68rem;color:#9ca3af">لا يوجد رابط</span>
                                        @endif
                                    </div>
                                </div>
                                <h4 style="font-size:0.9rem;font-weight:700;color:#111827;margin:0 0 4px;line-height:1.3;overflow:hidden;display:-webkit-box;-webkit-line-clamp:2;-webkit-box-orient:vertical" class="dark:text-white">
                                ...
                                <div style="display:flex;align-items:center;justify-content:space-between;padding-top:10px;border-top:1px solid #f1f5f9" class="dark:border-white/10">
                                    ...
                                    @if($fileUrl)
                                    <span style="font-size:0.75rem;font-weight:600;color:#6366f1;display:flex;align-items:center;gap:4px">
                                        <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2">
            ...
            <div style="background:#fff;border-radius:18px;border:1.5px solid #f1f5f9;padding:60px 40px;text-align:center" class="dark:bg-slate-800 dark:border-white/10">
                <div style="width:72px;height:72px;border-radius:20px;background:#f1f5f9;display:flex;align-items:center;justify-content:center;margin:0 auto 16px" class="dark:bg-white/10">
        ...
        <div class="preview-container">
            ...
            <div class="preview-header">
                <div style="width:36px;height:36px;border-radius:10px;display:flex;align-items:center;justify-content:center;flex-shrink:0"
                     :style="preview.type === 'pdf' ? 'background:#fee2e2' : 'background:#dbeafe'">
                ...
                <div style="display:flex;align-items:center;gap:8px">
                    ...
                    <button @click="closePreview()"
                            style="width:32px;height:32px;border-radius:8px;background:rgba(255,255,255,0.1);border:none;cursor:pointer;display:flex;align-items:center;justify-content:center;transition:background 0.15s;color:#fff"
                            onmouseover="this.style.background='rgba(255,255,255,0.2)'" onmouseout="this.style.background='rgba(255,255,255,0.1)'">
                ...
                <template x-if="preview.open && preview.type === 'video' && !isLocalVideo() && !isYouTube()">
                    <div style="display:flex;flex-direction:column;align-items:center;justify-content:center;min-height:300px;gap:16px;padding:40px">
                        ...
                        <p style="color:rgba(255,255,255,0.5);font-size:0.875rem;text-align:center;margin:0">
                            ...
                            <a :href="preview.url" target="_blank" style="color:#6366f1">افتح في تبويب جديد</a>
                        </p>
                    </div>
                </template>
            </div>
...
function filesApp() {
    return {
        ...
        isLocalVideo() {
            if (!this.preview.url) return false;
